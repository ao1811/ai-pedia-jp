...
tldr: "ChatGPT エージェントモードは万能ではない。送金・契約・人事評価・機密情報・法務系の5領域は2026年時点で「人間の最終承認なしには絶対に任せてはいけない」業務。仮想ブラウザの操作精度は90%程度で10%は誤動作する前提。情シス・法務観点でのチェックリストと、実際に編集部とクライアントで発生したトラブル事例を共有します。"
faq:
  ...
  - q: "実際のトラブル事例はある？"
    a: "編集部が把握している範囲で、誤った Notion ページの上書き、競合に類似機能を発注しかけた誤指示、求職者の個人情報を含む内部メモが Slack の他チャンネルに投稿される事例などが報告されています。"
...
ChatGPT エージェントモードは確かに業務効率化の革命ですが、**何でもかんでも任せて良いツールではありません**。本記事では、編集部および取引先で実際に発生した**トラブル事例**をもとに、2026年4月時点で「**やってはいけない5つの業務**」と、**安全運用のためのチェックリスト**を共有します。
...
ChatGPT エージェントモードの仮想ブラウザは進化していますが、**完全ではありません**。編集部が3ヶ月運用して把握した精度の実態：
