...
tldr: "Claude Code に投げるべき5つの『すぐ動く』定番プロンプトを実例つきで公開。①プロジェクト設計、②Content Collections のスキーマ設計、③SEO完璧化、④記事のテンプレート化、⑤キーワードクラスター一括生成。各プロンプトに『使うときのコツ』『カスタマイズ例』『実行結果』『ありがちな失敗』を詳述。本サイト ai-pedia.jp 立ち上げ時にも実際に活用したプロンプトです。"
faq:
...
「Claude Code に何を頼めばいいか分からない」——課金後の最大のつまずきです。本記事では、本サイト ai-pedia.jp 立ち上げ時に実際に投げて成果が出た **5つの定番プロンプト**を、**そのままコピペで使える形**で公開します。
...
- スタックを変えたい場合は「**Next.js 15**」や「**SvelteKit**」に置換可能（ただし Astro が SEO・速度で最有力）
...
- **Cloudflare Pages 以外を指定**：Vercel / Netlify でも動くが、Cloudflare Pages が個人サイト最有力コスパ

---

## 定番プロンプト2：Content Collections のスキーマ設計
...
| 日数 | 投入するプロンプト | 結果 |
|---|---|---|
| **Day 1（朝）** | 定番プロンプト1：プロジェクト設計 | プロジェクトの骨格完成（5時間） |
| **Day 1（昼）** | 定番プロンプト2：Content Collections | 型安全な記事管理基盤完成（1時間） |
| **Day 1（夜）** | 定番プロンプト3：SEO 完璧化 | 全 SEO 設定完了（半日） |
| **Day 2** | 定番プロンプト4：記事テンプレ | 以降の記事執筆が高速化（30分） |
| **Day 3〜5** | 定番プロンプト5：クラスター量産 | 1日1〜10クラスター完成 |
...
- **定番プロンプト1**：プロジェクト設計（5時間で骨格完成）
- **定番プロンプト2**：Content Collections（型安全な基盤）
- **定番プロンプト3**：SEO 完璧化（普通のブログの半年分を初日に）
- **定番プロンプト4**：記事テンプレート（量産しても品質維持）
- **定番プロンプト5**：キーワードクラスター（1日で10クラスター可能）
